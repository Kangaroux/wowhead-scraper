...
export async function scrapeNPCInfo(page: Page, id: number): Promise<NPC> {
    ...
    let factionHostility: FactionHostility = {
        alliance: "enemy",
        horde: "enemy",
    };
...
async function getFactionHostility(
    el: ElementHandle<HTMLElement>,
): Promise<FactionHostility> {
    const spans = await el.$$("span");

    const alliance = (await spans[0].evaluate((el) =>
        el.getAttribute("class"),
    )) as string;
    const horde = (await spans[1].evaluate((el) =>
        el.getAttribute("class"),
    )) as string;
